...
*   No muestra con la misma claridad los porcentajes que el
gráfico de barras.
...
```{r graficoPareto, echo=FALSE}

# Vector con los %
valores<-as.numeric(round(prop.table(table(df_cis$recPREFPTE))*100,                                                              digits=1))
# Vector con los nombres de líderes
nombres<-names(round(prop.table(table(df_cis$recPREFPTE))*100,digits=1))

# vector con los índices en orden decreciente (para ordenar)
orden<-order(valores,decreasing=TRUE)

# Se ordenan los dos vectores
valoresOrdenados<-valores[orden]
nombresOrdenados<-nombres[orden]

#fusionamos
ordenados<-valoresOrdenados
names(ordenados)<-nombresOrdenados

#colores=c("red","blue","#FF33FF","#2ecc71","#2980b9","#ab47bc","#c8e6c9")
#en Pareto usaremos un solo color
# Ahora ya creamos el gráfico de Pareto

p<-pareto.chart(ordenados, ylab = " % Preferencia", xlab = "",
...
```{r guardarTablaXLSXadd, echo=TRUE}
# file= nombre del fichero hoja de cálculo de MSOffice
write.xlsx(grafico, file = "DATOS/tablafrecResultado.xlsx",

...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# INSTALAR (si no están) PAQUETES de LIBRERÍAS R NECESARIAS }
\CommentTok{\# Se ocultan mensajes instalación(message=FALSE)}
...
\CommentTok{\# Para mejor la comprensión se puede cambiar los nombres de las columnas}
\CommentTok{\# e imprimir la tabla con Rmd}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{summary}\NormalTok{(df\_cisProbVoto}\SpecialCharTok{$}\NormalTok{recPROBVOTO,}\AttributeTok{digits=}\DecValTok{2}\NormalTok{) }\CommentTok{\# 2 dígitos significativos}
\end{Highlighting}
\end{Shaded}
...
\emph{frecuencias acumuladas} de la Tabla de frecuencias donde el valor
8 acumula el 8,5\%.

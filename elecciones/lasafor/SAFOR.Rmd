...
Se intenta cargar una librería mediante *require()* . Si da error, require devuelve un valor FALSE (!require()), entonces se ejecuta la instalación del paquete (que carga también la librería).
...
Veamos como ejemplo las primeras líneas...
...
## Vemos las primeras lineas de ejemplo
```{r 2b, include=TRUE}
head(df_autonomicas)
```
...
# CÁLCULO DE VALORES DE MEDIDAS de TENDENCIA CENTRAL y DISPERSIÓN
...
# GUARDAR DATA FRAME EN FICHEROS DE DIFERENTES FORMATOS

## Guardar datos en formato .xlsx (MS Excel)
...
## Añadiendo una hoja en un ficher xlsx existente
...
## Guardar datos en formato .dta / Stata.
```{r 10, include=FALSE}
...
## Guardar datos en formato .sav / SPSS
```{r 12, include=FALSE}
...
write_sav(df_resultats, "resultats.sav")
```
## Guardar y abrir datos en formato RStudio.
...
## RDS - guarda un solo archivo de datos
```{r 15, include=TRUE}
saveRDS(df_resultats, "resultats.rds")
```

## Lectura de datos RDS
```{r 16, include=TRUE}
